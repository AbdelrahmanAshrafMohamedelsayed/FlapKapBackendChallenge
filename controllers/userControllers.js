...
exports.deposit = catchAsync(async (req, res, next) => {
  ...
  if (!validAmounts.includes(amount))
    return next(
      new AppError('You can only deposit 5, 10, 20, 50, or 100 cent coins', 400)
    );

  await User.updateOne({ _id: req.user.id }, { $inc: { deposit: amount } });
...
exports.resetDeposit = catchAsync(async (req, res, next) => {
...
exports.buyProduct = catchAsync(async (req, res, next) => {
  ...
  if (+user.deposit < totalCost)
    return next(new AppError("You don't have enough deposit", 400));
  await User.updateOne({ _id: req.user.id }, { $inc: { deposit: -totalCost } });
  // decrement the amount of the product by the amount purchased
  ...
  let change = +updatedUser.deposit;
  const coins = [100, 50, 20, 10, 5];
  const changeInCoins = coins.map(coin => {
  ...
  res.status(200).json({
    ...
    data: {
      ...
      productsPurchased: {
        ...
        amount_left_of_product: updatedProduct.amountAvailable
      }
    }
  });
});
